...
{ "id":"a_1", "from":"n_1", "to":"n_2",
  ...
  "color":"blue", "style":"solid", "label":"1. HTTPS", "animated":false }
  // label MUST begin with the arrow's sequence badge (see SEQUENCE NUMBERING).
...
═══════════════════════════════════════════════════════════════
SEQUENCE NUMBERING (REQUIRED — so the order is readable WITHOUT pressing Play)
═══════════════════════════════════════════════════════════════

Every arrow's `label` MUST start with a sequence badge that reflects the
order of the flow, so a viewer can read the sequence visually on the static
diagram without running the play animation.

RULES:
- Number arrows in the same order they appear in `playFlow` — the badge on
  arrow a_N equals that arrow's position in the playFlow story (1, 2, 3, …).
- Badge format: the plain number, a period, a single space, THEN the
  connection label. Use NORMAL digits (1, 2, 3, …) — do NOT use circled
  glyphs like ①②③, they render much smaller than the label text.
  Examples: "1. HTTPS", "2. SQL query", "3. publish event".
- The badge and the label are ONE continuous text string with a space
  between them — same font size, rendered inline, never overlapping.
- If an arrow would otherwise have no label, the label is JUST the badge
  (e.g. "4."). Never leave label empty — the badge must always show.
- Keep the label after the badge SHORT (1–3 words: protocol/verb/payload).
  The full explanation belongs in `playFlow.text`, not on the arrow.
- Each badge number is unique. Parallel/branching arrows still get the next
  number in the order you want the viewer to read them.
- Every arrow appears in `playFlow` exactly once, and every `playFlow` step
  references an existing arrow — the two lists are the same set of hops in the
  same order. If a connection isn't worth narrating, don't draw it: there are
  no unnumbered, un-narrated arrows.
...
Common AWS paths:
  ...
  icons/aws/Containers/Amazon-Elastic-Container-Service.svg
  icons/aws/Containers/AWS-Fargate.svg
  icons/aws/Containers/Amazon-Elastic-Kubernetes-Service.svg
...
Match the icon to the EXACT service the user named — never to a similarly
spelled but different one. If a named service isn't in this library, OMIT
iconUrl; do not borrow the closest-looking icon. Watch near-name collisions:
"APIC" is IBM API Connect, NOT Amazon "API Gateway"; "Apigee" is Google's, not
Amazon's. A neutral emoji is better than a confidently wrong brand icon.
...
- The order of this array IS the sequence: the Nth entry's arrow must carry
  the badge "N." in its label (see SEQUENCE NUMBERING). Keep them in sync.
